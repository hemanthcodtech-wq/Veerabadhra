...
async function sendOTPEmail(email, otp, name) {
  await transporter.sendMail({
    ...
    to: email,
    subject: 'Your OTP for VEERABADHRA ENTERPRISE Signup',
      html: `
        <div style="font-family:serif;max-width:480px;margin:auto;padding:32px;border:1px solid rgba(212,175,55,0.3);border-radius:16px;background-color:#FDF7E5;box-shadow:0 8px 30px rgba(7,48,32,0.1);">
          <div style="text-align:center;margin-bottom:24px;">
            <h2 style="color:#073020;margin:0;font-size:26px;letter-spacing:1px;">VEERABADHRA ENTERPRISE</h2>
            <p style="color:#D4AF37;margin:8px 0 0 0;font-weight:bold;font-size:13px;letter-spacing:2px;text-transform:uppercase;">Premium Pooja Essentials</p>
          </div>
          <p style="color:#073020;font-size:16px;font-family:sans-serif;">Hi <strong>${name}</strong>,</p>
          <p style="color:#073020;font-size:15px;font-family:sans-serif;line-height:1.5;">Thank you for beginning your spiritual journey with us. Your OTP for account verification is:</p>
          <div style="font-size:40px;font-weight:bold;color:#D4AF37;letter-spacing:12px;text-align:center;padding:20px;background:#ffffff;border:2px solid #D4AF37;border-radius:12px;margin:24px 0;box-shadow:inset 0 2px 10px rgba(212,175,55,0.1);">
            ${otp}
          </div>
          <p style="color:#6b7280;font-size:13px;text-align:center;font-family:sans-serif;">This OTP is valid for 10 minutes. Please do not share it with anyone.</p>
        </div>
      `,
  });
}
...
async function sendSMSOTP(phone, otp, name) {
  ...
  try {
    await twilioClient.messages.create({
      body: `Your VEERABADHRA ENTERPRISE OTP is ${otp}. Valid for 10 minutes.`,
...
router.post('/signup', async (req, res) => {
  ...
  try {
    ...
    if (existing.rows.length) {
      await pool.query('UPDATE users SET name=$1, phone=$2, password_hash=$3, role=$5, is_verified=FALSE WHERE email=$4', [name, phone, hash, email, userRole]);
    } else {
      await pool.query('INSERT INTO users (name, email, phone, password_hash, role, is_verified) VALUES ($1,$2,$3,$4,$5,FALSE)', [name, email, phone, hash, userRole]);
    }
    // Send email OTP instead of phone OTP
    const emailOtp = generateOTP();
    const expiresAt = new Date(Date.now() + 10 * 60 * 1000);
    await pool.query('DELETE FROM otps WHERE email=$1', [email]);
    await pool.query('INSERT INTO otps (email, otp, expires_at) VALUES ($1,$2,$3)', [email, emailOtp, expiresAt]);
...
router.post('/verify-otp', async (req, res) => {
  ...
  try {
    ...
    const userCheck = await pool.query('SELECT is_verified FROM users WHERE email=$1', [email]);
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (!user.is_verified) {
      return res.status(403).json({ error: 'Please verify your account to continue', unverified: true });
    }
...
router.post('/forgot-password', async (req, res) => {
  ...
  try {
    ...
    await transporter.sendMail({
      from: `"VEERABADHRA ENTERPRISE" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: 'Reset Your VEERABADHRA ENTERPRISE Password',
      html: `
        <div style="font-family:serif;max-width:480px;margin:auto;padding:32px;border:1px solid rgba(212,175,55,0.3);border-radius:16px;background-color:#FDF7E5;box-shadow:0 8px 30px rgba(7,48,32,0.1);">
          <div style="text-align:center;margin-bottom:24px;">
            <h2 style="color:#073020;margin:0;font-size:26px;letter-spacing:1px;">VEERABADHRA ENTERPRISE</h2>
            <p style="color:#D4AF37;margin:8px 0 0 0;font-weight:bold;font-size:13px;letter-spacing:2px;text-transform:uppercase;">🔐 Password Reset</p>
          </div>
          <p style="color:#073020;font-size:16px;font-family:sans-serif;">Hi <strong>${result.rows[0].name}</strong>,</p>
          <p style="color:#073020;font-size:15px;font-family:sans-serif;line-height:1.5;">We received a request to reset your password. Your verification OTP is:</p>
          <div style="font-size:40px;font-weight:bold;color:#D4AF37;letter-spacing:12px;text-align:center;padding:20px;background:#ffffff;border:2px solid #D4AF37;border-radius:12px;margin:24px 0;box-shadow:inset 0 2px 10px rgba(212,175,55,0.1);">
            ${otp}
          </div>
          <p style="color:#6b7280;font-size:13px;text-align:center;font-family:sans-serif;">Valid for 10 minutes. Please do not share it with anyone.</p>
        </div>
      `,
    });
