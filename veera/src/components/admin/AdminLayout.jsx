...
export function AdminLayout({ children }) {
  ...
  useEffect(() => {
    ...
    if (!token) {
      ...
      setAdmin({ name: "Admin User", email: "admin@upraders.com" });
      return;
    }
  ...
  if (!admin) return (
    <div className="min-h-screen bg-[#FDF7E5] flex items-center justify-center">
      <div className="w-10 h-10 border-4 border-[#08183A]/20 border-t-[#08183A] rounded-full animate-spin" />
    </div>
  );

  return (
    <div className="min-h-screen bg-[#FDF7E5] flex">
      {/* Mobile Header */}
          ...
          {NAV.map((item) => (
            <Link key={item.href} to={item.href} onClick={() => setMobileOpen(false)}
              className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-sans font-medium transition-colors ${
                pathname === item.href ? "bg-[#08183A]/10 text-[#08183A]" : "text-[#08183A]/60 hover:text-[#08183A] hover:bg-[#FDF7E5]"
              }`}>
