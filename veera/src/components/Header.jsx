...
function DesktopSearchBar() {
  ...
  return (
    <div ref={ref} className="relative w-[280px] xl:w-[320px]">
      ...
      />
      {open && trimmed && (
        <div className="absolute top-full mt-2 left-0 w-full bg-white rounded-2xl shadow-xl border border-brand-green/10 overflow-hidden z-50">
          {results.length > 0 ? (
            <>
              {results.map(p => {
              ...
              <button onClick={() => { setOpen(false); navigate(`/search?q=${encodeURIComponent(trimmed)}`); }}
                className="w-full text-center text-xs font-bold text-brand-green py-2.5 border-t border-brand-green/10 hover:bg-brand-beige transition-colors">
...
function DesktopFullHeader({ cartCount, wishlistCount, token, user, handleLogout, categories, offers, announcement, locationName, isLoadingLocation, fetchLocation }) {
  return (
    <>
      ...
      <header className={`fixed ${announcement && announcement.is_active && announcement.items?.some(i => i.text) ? 'top-[32px]' : 'top-0'} left-0 z-50 w-full bg-[#073020] px-4 md:px-12 lg:px-20 py-2 shadow-md border-b border-[#041f15] hidden md:block transition-all`}>
        <div className="w-full mx-auto flex items-center justify-between">

          <div className="flex items-center gap-6">
            <div className="flex items-center gap-3 hover:opacity-95 transition-opacity">
              <Link to="/" className="h-[60px] w-[60px] shrink-0 flex items-center justify-center drop-shadow-[0_0_16px_rgba(212,175,55,0.7)] transition-transform hover:scale-105">
                <img src={logo} alt="VEERABADHRA ENTERPRISE Logo" className="h-full w-full object-contain" />
              </Link>
              <div className="flex flex-col text-left mt-1">
                <Link to="/">
                  <span className="font-serif font-bold text-[13px] xl:text-lg leading-[1.1] tracking-wide text-white block whitespace-normal w-[110px] xl:w-auto">VEERABADHRA ENTERPRISE</span>
                </Link>
          ...
          <div className="flex-1 flex items-center justify-end md:justify-center px-4 lg:px-8 xl:px-12 gap-4 xl:gap-8">
            <nav className="hidden lg:flex items-center gap-6">
              <Link to="/" className="text-sm font-medium text-white/90 hover:text-[#D4AF37] transition-colors">Home</Link>
              <div className="relative group">
                <div className="flex items-center cursor-pointer py-4">
                  ...
                  <ChevronDown className="w-4 h-4 ml-1 text-white/90 group-hover:text-[#D4AF37] transition-transform group-hover:-rotate-180" />
                </div>
              ...
              <Link to="/wishlist" className="text-sm font-medium text-white/90 hover:text-[#D4AF37] transition-colors">Wishlist</Link>
              <Link to="/about" className="text-sm font-medium text-white/90 hover:text-[#D4AF37] transition-colors">About</Link>
              <Link to="/contact" className="text-sm font-medium text-white/90 hover:text-[#D4AF37] transition-colors">Contact</Link>
            </nav>
            <DesktopSearchBar />
          </div>
          ...
          <div className="flex items-center gap-3">
            <Link to="/wishlist" className="relative p-1 cursor-pointer hover:-translate-y-0.5 transition-transform">
              ...
              {wishlistCount > 0 && (
                <span className="absolute top-0 right-0 bg-[#D4AF37] text-white text-[10px] font-bold w-4 h-4 flex items-center justify-center rounded-full border border-[#073020]">
                  {wishlistCount}
                </span>
              )}
            </Link>
            ...
            {token ? (
              ...
              <Link to="/login" className="flex items-center gap-1.5 text-xs font-bold text-white bg-[#D4AF37] px-4 py-2 rounded-lg hover:bg-[#D4AF37]/90 transition-colors ml-2 shadow-sm">
        ...
        <>
          ...
          >
            ...
            <motion.div
              ...
              className="px-5 py-5 flex items-center justify-between border-b border-brand-green/10 bg-white"
            >
              <div className="flex items-center gap-3 w-[85%]">
                <div className="h-20 w-20 shrink-0 flex items-center justify-center drop-shadow-[0_0_8px_rgba(212,175,55,0.2)]">
                  <img src={logo} alt="VEERABADHRA ENTERPRISE" className="w-full h-full object-contain" />
                </div>
                <div className="flex flex-col text-left">
                  <span className="font-serif font-bold text-[13px] leading-tight text-brand-green uppercase">VEERABADHRA<br/>ENTERPRISE</span>
                </div>
              </div>
              <button
                onClick={() => setMobileMenuOpen(false)}
                  ...
                  {label === 'Categories' ? (
                    <div className="flex flex-col rounded-xl overflow-hidden">
                      <div
                        ...
                        className="flex items-center justify-between hover:bg-white hover:text-brand-green transition-all cursor-pointer group rounded-xl"
                      >
                        <span className="block text-gray-900 font-semibold text-base py-3 px-4 flex-grow group-hover:text-brand-green">
                          {label}
                        </span>
                        <button className="p-3 text-gray-900 group-hover:text-brand-green">
                    ...
                    <Link
                      to={to}
                      onClick={() => setMobileMenuOpen(false)}
                      className="block text-gray-900 font-semibold text-base py-3 px-4 rounded-xl hover:bg-white hover:text-brand-green transition-all"
                    >
                      {label}
                    </Link>
                  )}
            ...
            {token ? (
              <motion.div
                ...
                className="p-4 border-t border-brand-green/10"
              >
              ...
              >
                <Link
                  to="/login"
                  onClick={() => setMobileMenuOpen(false)}
                  className="flex items-center justify-center gap-2 w-full bg-white text-brand-green font-bold py-3.5 rounded-xl shadow-sm hover:bg-white transition-all"
                >
  ...
  return (
    <>
      ...
      <div className="md:hidden">
        ...
        <header className={`fixed ${announcement && announcement.is_active && announcement.items?.some(i => i.text) && variant !== 'home' ? 'top-[27px]' : 'top-0'} left-0 z-50 w-full ${variant === 'home' ? 'bg-[#073020] pt-2 pb-4 px-4 h-auto' : 'bg-[#073020] backdrop-blur-md px-4 py-2 shadow-lg border-b border-[#041f15] h-[76px] transition-all'}`}>
          {variant === 'home' ? (
            <div className="w-full flex flex-col gap-3">
              <div className="flex items-center justify-between w-full mb-1">
                <div className="flex items-center gap-2">
                  ...
                  <Link to="/" className="flex items-center gap-2 transition-transform active:scale-95">
                    <img src={logo} alt="Logo" className="h-14 w-14 sm:h-16 sm:w-16 shrink-0 object-contain drop-shadow-[0_0_10px_rgba(212,175,55,0.5)]" />
                    <div className="flex flex-col -mt-0.5">
                      <span className="font-serif font-bold text-[13px] sm:text-[14px] leading-tight text-white tracking-wide">VEERABADHRA</span>
                      <span className="font-serif font-bold text-[9px] sm:text-[10px] leading-tight text-[#D4AF37] tracking-[0.2em] uppercase">Enterprise</span>
                    </div>
                  </Link>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <Link to="/cart" className="relative bg-[#D4AF37] rounded-full p-2.5 text-white flex-shrink-0 cursor-pointer hover:scale-105 transition-transform shadow-sm">
                    <ShoppingCart className="w-4 h-4 md:w-5 md:h-5" />
                    {cartCount > 0 && (
                      <span className="absolute -top-1 -right-1 bg-white text-[#D4AF37] text-[9px] font-bold w-4 h-4 flex items-center justify-center rounded-full border-2 border-[#073020]">
                        {cartCount}
                      </span>
                    )}
                  </Link>
              ...
              >
                ...
                />
                <Mic className="w-5 h-5 text-brand-green cursor-pointer" />
              </form>
            </div>
          ) : (
                  ...
                  {mobileSearchResults.length > 0 ? (
                    <>
                      ...
                      <button
                        ...
                        className="w-full text-center text-xs font-bold text-brand-green py-3 border-t border-brand-green/10 hover:bg-brand-beige transition-colors">
