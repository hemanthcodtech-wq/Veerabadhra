...
export function BottomNav() {
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 w-full bg-white backdrop-blur-lg border-t border-brand-green/10 rounded-t-3xl shadow-[0_-8px_30px_rgba(0,0,0,0.2)] pb-safe z-50 transition-all">
          ...
          return (
            <NavLink key={tab.name} to={tab.path}
              ...
              className={cn(
                ...
                isActive ? 'text-brand-green -translate-y-1' : 'text-gray-400 hover:text-gray-900'
              )}>
              <div className={cn(
                "p-1.5 rounded-full transition-all duration-300",
                isActive ? "text-brand-green" : "bg-transparent"
              )}>
              ...
              {isActive && (
                <div className="absolute -bottom-2 w-1 h-1 bg-brand-green text-gray-900 rounded-full shadow-[0_0_8px_rgba(212,175,55,0.6)]"></div>
              )}
            </NavLink>
          );
        })}
