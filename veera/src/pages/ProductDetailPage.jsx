...
export function ProductDetailPage() {
  ...
  if (!product) return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-brand-beige text-gray-900 gap-4">
      ...
      <button onClick={() => navigate('/')} className="bg-brand-green text-white px-8 py-3 rounded-2xl font-bold">Back to Home</button>
    </div>
  );
      ...
      <div className="md:hidden">
        ...
        <div className="bg-gradient-to-b from-brand-beige to-white relative pt-4 pb-10">
          ...
          <div className="flex items-center justify-center px-5 mt-6 relative z-0">
            <button
              ...
              className="relative group focus:outline-none bg-white w-full max-w-[340px] aspect-[4/3] rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-brand-green/5 flex items-center justify-center transition-transform active:scale-95"
            >
              <motion.img
                key={mainImg}
              ...
              />
              {/* Zoom hint overlay */}
              <span className="absolute bottom-3 right-3 bg-white/90 backdrop-blur rounded-full p-2.5 shadow-sm text-gray-500 hover:text-brand-green transition-colors">
        ...
        <div className="bg-white rounded-t-[32px] -mt-6 relative z-10 px-5 pt-6">
                  ...
                  return (
                    <button key={i} onClick={() => handleColorChange(v.color)}
                      className={`px-4 py-2 rounded-xl border-2 text-sm font-bold transition-all ${
                        isSelected
                          ? 'border-[#073020] text-[#073020] bg-red-50'
                  ...
                  return (
                    <button key={i} onClick={() => setSelectedSize(s)}
                      className={`px-5 py-2.5 rounded-full border-2 text-sm font-bold transition-all ${
                        isSelected
                          ? 'bg-[#073020] border-[#073020] text-white shadow-md scale-105'
          ...
          <div className="grid grid-cols-2 gap-2 mt-6">
            {[
              { icon: <Truck className="w-4 h-4 text-[#073020]" />, label: 'Fast Delivery' },
              { icon: <RotateCcw className="w-4 h-4 text-[#073020]" />, label: 'Easy Returns' },
              { icon: <Check className="w-4 h-4 text-[#073020]" />, label: '100% Quality' },
                ...
                {relatedProducts.map(p => {
                  ...
                  return (
                    <button key={p.id} onClick={() => navigate(`/product/${p.id}`)} className="flex-shrink-0 w-28 group">
                      <div className="w-28 h-28 bg-brand-beige rounded-2xl overflow-hidden border border-brand-green/10 group-hover:scale-105 transition-transform">
            ...
            <div className="mt-6 bg-gray-50 rounded-2xl p-4 mb-4">
              ...
              {reviewSuccess ? (
                ...
                <form onSubmit={handleReviewSubmit} className="space-y-3">
                  ...
                  <button type="submit" disabled={reviewSubmitting} className="w-full py-3 bg-[#073020] text-white font-bold rounded-xl text-sm disabled:opacity-60">
      ...
      <div className="hidden md:block bg-gray-50 min-h-screen pt-4 pb-20">
        <div className="max-w-6xl mx-auto px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2 text-xs font-semibold text-gray-500 py-4">
            <button onClick={() => navigate('/')} className="hover:text-[#073020] transition-colors">Home</button>
            <span>/</span>
            <button onClick={() => navigate(`/category/${product.category}`)} className="capitalize hover:text-[#073020] transition-colors">
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 lg:p-10 mb-8 grid grid-cols-12 gap-10">
            ...
            <div className="col-span-5 flex flex-col items-center">
              <div className="relative w-full h-[380px] bg-brand-beige/50 rounded-2xl p-6 flex items-center justify-center border border-brand-green/10 group">
                <img
                ...
                >
                  <ZoomIn className="w-4 h-4 text-[#073020]" /> Zoom
                </button>
              </div>
                  ...
                  {images.map((img, i) => (
                    <button
                      ...
                      className={`w-16 h-16 rounded-xl overflow-hidden border-2 transition-all shrink-0 ${
                        mainImg === img ? 'border-[#073020] shadow-md scale-105' : 'border-gray-200 opacity-70 hover:opacity-100'
                      }`}
                    >
                      <img src={img} alt="" className="w-full h-full object-cover" />
                    </button>
            ...
            <div className="col-span-7 flex flex-col justify-between">
              <div>
                ...
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-bold text-[#073020] bg-red-50 px-3 py-1 rounded-full uppercase tracking-wider">
                        ...
                        return (
                          <button
                            ...
                            className={`px-5 py-2.5 rounded-xl border-2 text-sm font-bold transition-all ${
                              isSelected
                                ? 'border-[#073020] text-[#073020] bg-red-50 shadow-sm'
                      ...
                      {sizes.map((s, i) => {
                        ...
                        return (
                          <button
                            ...
                            className={`px-5 py-2.5 rounded-xl border-2 text-sm font-bold transition-all ${
                              isSelected
                                ? 'bg-[#073020] border-[#073020] text-white shadow-md'
                                : 'border-gray-200 text-gray-700 bg-white hover:border-gray-300'
                            }`}
                          >
                            {s.size}
                  ...
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2.5 py-3.5 rounded-2xl font-bold text-base transition-all ${
                      isOutOfStock
                        ...
                        : 'bg-[#073020] text-white hover:bg-[#b81633] shadow-lg shadow-[#073020]/20 active:scale-[0.98]'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`px-8 py-3.5 rounded-2xl font-bold text-base border-2 transition-all ${
                      isOutOfStock
                        ...
                        : 'border-[#073020] text-[#073020] hover:bg-red-50 active:scale-[0.98]'
                    }`}
                  >
                    Buy Now
                  </button>
              ...
              <div className="col-span-4 bg-gray-50 rounded-2xl p-5 border border-gray-100 h-fit">
                ...
                <div className="divide-y divide-gray-200 text-xs">
                  ...
                  <div className="py-2 flex justify-between">
                    ...
                    <span className="text-gray-900 font-bold">VEERABADHRA ENTERPRISE</span>
                  </div>
                </div>
              </div>
            </div>
                    ...
                    <form onSubmit={handleReviewSubmit} className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <input
                          ...
                          className="px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:border-[#073020] bg-white"
                        />
                      ...
                      <textarea
                        ...
                        className="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:border-[#073020] bg-white resize-none"
                      />
                      <button
                        type="submit"
                        disabled={reviewSubmitting}
                        className="px-6 py-2.5 bg-[#073020] text-white font-bold rounded-xl text-sm hover:bg-[#b81633] transition-colors disabled:opacity-60"
                      >
                        {reviewSubmitting ? 'Submitting...' : 'Submit Review'}
                      </button>
                    </form>
      ...
      <div className="fixed bottom-0 left-0 w-full z-[60] md:hidden">
        <div className="bg-white border-t border-gray-100 px-4 py-3 flex items-center gap-3 shadow-2xl">
          ...
          <button onClick={handleAddToCart} disabled={isOutOfStock}
            className={`flex-1 flex items-center justify-center gap-2 font-bold py-4 rounded-2xl text-base transition-all active:scale-95 ${isOutOfStock ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : addedFeedback ? 'bg-green-500 text-white' : 'bg-[#073020] text-white shadow-lg shadow-[#073020]/30'}`}>
            <ShoppingCart className="w-5 h-5" />
            {isOutOfStock ? 'Out of Stock' : addedFeedback ? '✓ Added to Cart!' : 'Add to Cart'}
          </button>
        </div>
