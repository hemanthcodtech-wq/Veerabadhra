        ...
        <React.Fragment key={i}>
          <div className="flex flex-col items-center gap-1 flex-shrink-0">
            <div className={`${mobile ? 'w-8 h-8' : 'w-7 h-7'} rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 ${
              ...
              i === activeIdx ? 'bg-[#073020] text-white ring-4 ring-[#073020]/20' :
            ...
            <span className={`${mobile ? 'text-[10px]' : 'text-[9px]'} font-semibold tracking-wide uppercase ${
              i === activeIdx ? 'text-[#073020]' : i < activeIdx ? 'text-green-600' : 'text-gray-300'
            }`}>{label}</span>
          </div>
          {i < steps.length - 1 && (
  ...
  return (
    <button
      ...
      className={`group relative w-full flex flex-col items-center gap-3 ${mobile ? 'p-5' : 'p-6'} rounded-2xl border-2 transition-all duration-300 cursor-pointer active:scale-95 ${
        selected
          ? 'border-[#073020] bg-[#FDF7E5] shadow-[0_0_15px_rgba(212,175,55,0.3)] ring-4 ring-[#073020]/10'
          : 'border-[#D4AF37]/30 bg-white/50 backdrop-blur-sm hover:border-[#073020]/40 hover:shadow-md'
      }`}
    >
      <div className={`${mobile ? 'w-14 h-14' : 'w-16 h-16'} rounded-2xl flex items-center justify-center transition-all duration-300 ${
        selected ? 'bg-[#073020] shadow-lg shadow-[#073020]/30' : 'bg-gray-100 group-hover:bg-[#073020]/10'
      }`}>
        {isShopkeeper
          ? <Store className={`${mobile ? 'w-7 h-7' : 'w-8 h-8'} transition-colors ${selected ? 'text-white' : 'text-gray-400 group-hover:text-[#073020]'}`} />
          : <UserCircle2 className={`${mobile ? 'w-7 h-7' : 'w-8 h-8'} transition-colors ${selected ? 'text-white' : 'text-gray-400 group-hover:text-[#073020]'}`} />
        }
      </div>
      <div className="text-center">
        <p className={`${mobile ? 'text-base' : 'text-lg'} font-bold transition-colors ${selected ? 'text-[#073020]' : 'text-[#073020]'}`}>
      ...
      {selected && (
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
          className="absolute top-2.5 right-2.5 w-5 h-5 rounded-full bg-[#073020] flex items-center justify-center">
...
// ── OTP Input Row ─────────────────────────────────────────────────────────────
function OtpRow({ otp, setOtp, mobile = false }) {
    ...
    <div className="flex justify-center gap-2.5">
      {otp.map((digit, idx) => (
        <input
          ...
          type="text" inputMode="numeric" autoComplete="one-time-code" maxLength={1} value={digit}
          ...
          onChange={e => {
            ...
            if (val.length > 1) {
              const next = [...otp];
              val.slice(0, 6).split('').forEach((char, i) => { if (idx + i < 6) next[idx + i] = char; });
              setOtp(next);
              const nextFocus = Math.min(idx + val.length, 5);
              setTimeout(() => refs.current[nextFocus]?.focus(), 10);
            } else {
              const next = [...otp]; next[idx] = val.slice(-1); setOtp(next);
              if (val && idx < 5) setTimeout(() => refs.current[idx + 1]?.focus(), 10);
            }
          }}
          ...
          className={`${mobile ? 'w-12 h-14' : 'w-11 h-13'} text-center text-2xl font-bold rounded-xl border-2 bg-white text-[#073020] focus:outline-none focus:border-[#073020] focus:ring-4 focus:ring-[#073020]/10 transition-all duration-200 border-gray-200 hover:border-gray-300 shadow-sm`}
        />
      ))}
    </div>
  );
...
export function SignupPage() {
  ...
  const inputCls = "w-full bg-white border border-gray-200 shadow-sm rounded-xl px-4 pl-11 py-3 text-sm text-[#073020] placeholder:text-gray-400 focus:outline-none focus:border-[#073020] focus:ring-4 focus:ring-[#073020]/10 hover:border-gray-300 transition-all duration-200";

  const RoleBadge = ({ onClick }) => selectedRole ? (
    <button type="button" onClick={onClick || (() => setStep('role'))}
      className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#073020]/10 text-[#073020] text-xs font-semibold border border-[#073020]/20 hover:bg-[#073020]/20 transition-colors mb-2">
      {selectedRole === 'shopkeeper' ? <Store className="w-3 h-3" /> : <UserCircle2 className="w-3 h-3" />}
      {selectedRole === 'shopkeeper' ? 'Shopkeeper' : 'Customer'}
      <span className="text-[#073020]/50 ml-0.5">· change</span>
    </button>
  ) : null;
      ...
      <motion.div key={step} variants={slideVariants} initial="enter" animate="center" exit="exit"
        ...
        {step === 'role' && (
          <div className="space-y-5">
            <div className="text-center">
              <h2 className="text-2xl font-extrabold text-[#073020] tracking-tight">Join as</h2>
            ...
            <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }}
              ...
              className="w-full bg-gradient-to-r from-[#D4AF37] to-[#C5A030] text-[#073020] font-bold py-3.5 rounded-xl text-sm shadow-md hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 flex items-center justify-center gap-2">
            ...
            <button type="button" onClick={() => loginWithGoogle()}
              className="w-full bg-white border border-gray-200 text-[#073020] font-semibold py-3 rounded-xl flex items-center justify-center gap-3 text-sm hover:bg-gray-50 hover:border-gray-300 transition-all duration-300 shadow-sm">
              <GoogleSvg /> Continue with Google
            </button>
            <p className="text-center text-sm text-gray-500">
              Already have an account?{' '}
              <Link to="/login" className="font-bold text-[#073020] hover:text-[#073020] transition-colors">Sign In</Link>
            </p>
          </div>
        )}
        ...
        {step === 'form' && (
          <div className="space-y-1">
            <div className="mb-4">
              <RoleBadge />
              <h2 className="text-2xl font-extrabold text-[#073020] tracking-tight">Your Details</h2>
            ...
            <form onSubmit={handleSignup} className="space-y-3">
              <div>
                ...
                <div className="relative group">
                  <User className="w-4 h-4 text-gray-400 absolute left-4 top-1/2 -translate-y-1/2 group-focus-within:text-[#073020] transition-colors" />
              ...
              <div>
                ...
                <div className="relative group">
                  <Mail className="w-4 h-4 text-gray-400 absolute left-4 top-1/2 -translate-y-1/2 group-focus-within:text-[#073020] transition-colors" />
              ...
              <div>
                ...
                <div className="relative group">
                  <Lock className="w-4 h-4 text-gray-400 absolute left-4 top-1/2 -translate-y-1/2 group-focus-within:text-[#073020] transition-colors" />
                  ...
                  <button type="button" onClick={() => setShowPass(p => !p)}
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#073020] transition-colors">
              ...
              <label className="flex items-start gap-2.5 cursor-pointer pt-1">
                <input type="checkbox" checked={consentAccepted} onChange={e => setConsentAccepted(e.target.checked)} className="mt-0.5 w-4 h-4 accent-[#073020] rounded shrink-0" />
                <span className="text-xs text-gray-500 leading-relaxed">
                  I confirm I am 13+ and agree to the{' '}
                  <Link to="/terms-of-service" target="_blank" className="font-bold text-[#073020] underline hover:text-[#073020]">Terms & Conditions</Link> and{' '}
                  <Link to="/privacy-policy" target="_blank" className="font-bold text-[#073020] underline hover:text-[#073020]">Privacy Policy</Link>.
                </span>
              </label>
              ...
              <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }}
                ...
                className="w-full bg-gradient-to-r from-[#D4AF37] to-[#C5A030] text-[#073020] font-bold py-3.5 rounded-xl text-sm shadow-md hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 disabled:opacity-60 flex items-center justify-center gap-2">
        ...
        {step === 'otp' && (
          <div className="space-y-6">
            <div className="text-center">
              <RoleBadge />
              <div className="w-16 h-16 rounded-2xl bg-[#073020]/10 flex items-center justify-center mx-auto mb-4">
                <Mail className="w-8 h-8 text-[#073020]" />
              </div>
              <h2 className="text-2xl font-extrabold text-[#073020] tracking-tight">Check your email</h2>
              <p className="text-sm text-gray-500 mt-2">We sent a 6-digit code to <span className="font-semibold text-[#073020]">{form.email}</span></p>
            </div>
        ...
        {step === 'done' && (
          <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
            ...
            <div>
              <h2 className="text-2xl font-extrabold text-[#073020] tracking-tight">Welcome aboard! 🎉</h2>
      ...
      <motion.div key={step} variants={slideVariants} initial="enter" animate="center" exit="exit"
        ...
        {step === 'role' && (
          <div className="space-y-4">
            ...
            <motion.button whileTap={{ scale: 0.97 }}
              type="button" onClick={handleRoleContinue}
              className="w-full bg-gradient-to-r from-[#D4AF37] to-[#C5A030] text-[#073020] font-bold py-4 rounded-2xl text-base shadow-lg shadow-red-200 flex items-center justify-center gap-2 active:brightness-95 transition-all">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-[#D4AF37] text-[11px] font-bold tracking-widest uppercase">OR</span>
              <div className="h-px bg-[#D4AF37]/30 flex-1" />
            </div>
            <button type="button" onClick={() => loginWithGoogle()}
              className="w-full bg-white/50 backdrop-blur-md border border-[#D4AF37]/50 text-[#073020] font-bold py-3.5 rounded-2xl flex items-center justify-center gap-3 text-sm shadow-sm hover:bg-white transition-all">
              <GoogleSvg /> Continue with Google
            </button>

            <p className="text-center text-sm text-gray-500 pb-2">
              Have an account?{' '}
              <Link to="/login" className="text-[#D4AF37] font-bold hover:text-[#073020] transition-colors underline decoration-2 underline-offset-4">Sign In</Link>
            </p>
          </div>
        )}
            ...
            <form onSubmit={handleSignup} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input name="name" value={form.name} onChange={handleChange} required
                    ...
                    className="w-full bg-white border border-gray-200 rounded-2xl px-4 pl-11 py-3.5 text-[15px] text-[#073020] placeholder:text-gray-400 focus:outline-none focus:border-[#073020] focus:ring-4 focus:ring-[#073020]/10 shadow-sm transition-all" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input name="password" type={showPass ? 'text' : 'password'} value={form.password}
                    onChange={handleChange} onBlur={() => setPasswordTouched(true)} required
                    placeholder="Min 8 chars, 1 number, 1 special"
                    className={`w-full bg-white border rounded-2xl px-4 pl-11 pr-12 py-3.5 text-[15px] text-[#073020] placeholder:text-gray-400 focus:outline-none focus:ring-4 shadow-sm transition-all ${
                      passwordTouched && !passwordValid
                        ? 'border-red-400 focus:border-red-400 focus:ring-red-100'
                        : 'border-gray-200 focus:border-[#073020] focus:ring-[#073020]/10'
                    }`} />
                  <button type="button" onClick={() => setShowPass(p => !p)}
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#073020] p-1 transition-colors">
              ...
              <label className="flex items-start gap-3 cursor-pointer bg-gray-50 rounded-2xl p-3.5 border border-gray-200 active:bg-gray-100 transition-colors">
                <input type="checkbox" checked={consentAccepted} onChange={e => setConsentAccepted(e.target.checked)}
                  className="mt-0.5 w-5 h-5 accent-[#073020] rounded shrink-0" />
                <span className="text-xs text-gray-500 leading-relaxed">
              ...
              <motion.button whileTap={{ scale: 0.97 }}
                type="submit" disabled={loading}
                className="w-full bg-gradient-to-r from-[#D4AF37] to-[#C5A030] text-[#073020] font-bold py-4 rounded-2xl text-base shadow-lg shadow-red-200 disabled:opacity-60 flex items-center justify-center gap-2 active:brightness-95 transition-all">
        ...
        {step === 'otp' && (
          <div className="space-y-6">
            ...
            <div className="text-center">
              <RoleBadge />
              <div className="w-20 h-20 rounded-3xl bg-gradient-to-br from-[#073020]/10 to-rose-100 flex items-center justify-center mx-auto mb-5 shadow-inner">
                <Mail className="w-10 h-10 text-[#073020]" />
              </div>
              <h2 className="text-2xl font-extrabold text-[#073020] tracking-tight">Check your email</h2>
              <p className="text-sm text-gray-500 mt-2 leading-relaxed">
                We sent a 6-digit code to{' '}
                <span className="font-bold text-[#073020] block mt-0.5">{form.email}</span>
              </p>
            </div>

            {/* OTP boxes */}
            <div className="pt-2 pb-1">
              <OtpRow otp={otp} setOtp={setOtp} mobile={true} />
            </div>
        ...
        {step === 'done' && (
          <motion.div initial={{ scale: 0.85, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
            ...
            <div>
              <h2 className="text-3xl font-extrabold text-[#073020] tracking-tight">Welcome! 🎉</h2>
  ...
  return (
    <>
      {/* ════════════════ DESKTOP (lg+) ════════════════ */}
      <div className="hidden lg:flex h-screen w-full font-sans bg-[#073020] relative items-center justify-center overflow-hidden py-4">
        <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1605806616949-1e87b487cb2a?q=80&w=2070')] bg-cover bg-center opacity-40 mix-blend-overlay"></div>
        <div className="absolute inset-0 bg-gradient-to-t from-[#073020] via-transparent to-transparent"></div>
        <div className="w-full max-w-md bg-[#FDF7E5]/95 backdrop-blur-2xl rounded-[2rem] shadow-[0_0_50px_rgba(212,175,55,0.2)] border border-[#D4AF37]/30 flex flex-col items-center px-8 py-8 overflow-y-auto max-h-[94vh] relative z-10">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.4, ease: 'easeOut' }} className="w-full">
            <div className="flex flex-col items-center mb-5">
              <Link to="/login">
                <img src={logoImg} alt="VEERABADHRA ENTERPRISE" className="h-[120px] w-[120px] drop-shadow-[0_0_16px_rgba(212,175,55,0.7)] hover:scale-105 transition-transform" />
              </Link>
            </div>
      ...
      {/* ════════════════ MOBILE (< lg) ════════════════ */}
      <div className="lg:hidden min-h-screen w-full font-sans flex flex-col bg-[#073020] relative">
        <div className="fixed inset-0 bg-[url('https://images.unsplash.com/photo-1605806616949-1e87b487cb2a?q=80&w=2070')] bg-cover bg-center opacity-30 mix-blend-overlay pointer-events-none"></div>
        <div className="fixed inset-0 bg-gradient-to-b from-transparent via-[#073020]/80 to-[#073020] pointer-events-none"></div>

        {/* ── Gradient hero header ── */}
        <div className="relative px-6 pt-10 pb-16 overflow-hidden z-10">
          {/* Decorative circles */}
          <div className="absolute -top-8 -right-8 w-40 h-40 rounded-full bg-[#D4AF37]/5" />
          <div className="absolute top-10 -right-4 w-20 h-20 rounded-full bg-[#D4AF37]/10" />
          <div className="absolute -bottom-4 left-8 w-28 h-28 rounded-full bg-[#D4AF37]/5" />

          {/* Logo */}
          <div className="relative z-10 inline-block py-1.5 px-3 drop-shadow-[0_0_15px_rgba(212,175,55,0.6)]">
            <img src={logoImg} alt="VEERABADHRA ENTERPRISE Logo" className="h-[110px] w-auto object-contain" />
          </div>
        ...
        <div className="flex-1 -mt-8 relative z-10 flex flex-col">
          <div className="relative z-10 flex-1 bg-[#FDF7E5]/95 backdrop-blur-xl rounded-t-[2.5rem] px-6 pt-8 pb-12 shadow-[0_-10px_40px_rgba(212,175,55,0.15)] border-t border-[#D4AF37]/30 flex flex-col min-h-full">
            {/* Step bar */}
