    ...
    <div className="w-full max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-brand-green/10 shadow-sm overflow-hidden">
        ...
        <div className="p-6 space-y-5">
          {/* Enable toggle */}
          <div className="flex items-center gap-3 bg-brand-green text-white/5 p-4 rounded-xl">
            <input
              type="checkbox"
          ...
          <div className="space-y-3">
            {announcement.items.map((item, idx) => (
              <div key={idx} className="flex gap-3 items-start bg-[#FDF7E5] border border-brand-green/10 rounded-xl p-4">
                <div className="flex-1 space-y-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 rounded-lg bg-white border border-brand-green/20 focus:outline-none focus:border-[#D4AF37] text-gray-900 text-sm"
                    />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-gray-900/60 mb-1 flex items-center gap-1.5">
          ...
          <button
            onClick={addItem}
            className="flex items-center gap-2 text-sm font-bold text-gray-900/70 hover:text-gray-900 border border-dashed border-brand-green/20 hover:border-brand-green/40 px-4 py-2.5 rounded-xl transition-colors w-full justify-center"
          >
          ...
          <div className="pt-2">
            <button
              onClick={handleSave}
              disabled={saving}
              className="bg-brand-green text-white hover:bg-brand-orange text-white text-white px-6 py-2.5 rounded-xl font-bold transition-colors flex items-center gap-2 disabled:opacity-50"
            >
