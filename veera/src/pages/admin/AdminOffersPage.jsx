      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {offers.map((offer, i) => (
          <motion.div key={offer.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
            className="bg-white rounded-2xl border border-brand-green/10 p-5 shadow-sm relative overflow-hidden flex flex-col justify-between">
            {!offer.is_active && (
            ...
            <div>
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleEdit(offer)} className="text-gray-900 hover:bg-brand-green text-white/10 p-1.5 rounded transition-colors"><Edit2 className="w-4 h-4" /></button>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden">
            ...
            <div className="border-t border-brand-green/10 px-6 py-4 flex gap-3">
              <button onClick={() => setEditOffer(null)} className="flex-1 px-4 py-2 bg-[#FDF7E5] text-gray-900 rounded-xl font-semibold">Cancel</button>
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto">
              <div className="flex border-b border-gray-200 mb-4">
                <button 
                  className={`flex-1 py-2 font-semibold text-sm ${applyMode === 'category' ? 'border-b-2 border-brand-green text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}
                  onClick={() => setApplyMode('category')}
                >
                  By Category
                </button>
                <button 
                  className={`flex-1 py-2 font-semibold text-sm ${applyMode === 'products' ? 'border-b-2 border-brand-green text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}
            ...
            <div className="border-t border-brand-green/10 px-6 py-4 flex gap-3 shrink-0">
              <button onClick={() => { setApplyOfferId(null); setSelectedCategory(""); setSelectedProducts([]); }} className="flex-1 px-4 py-2 bg-[#FDF7E5] text-gray-900 rounded-xl font-semibold">Cancel</button>
              <button onClick={handleApplyAction} className="flex-1 px-4 py-2 bg-brand-green text-white rounded-xl font-semibold">
                Apply Offer
              </button>
            </div>
          </div>
