    ...
    <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button onClick={() => setShowMarkPaid(p => !p)}
          className="flex items-center gap-1.5 bg-brand-green text-white hover:bg-brand-green text-white/80 text-white text-xs font-bold px-3 py-2 rounded-lg transition-colors">
          ✓ Mark as Paid
        </button>
      </div>
            ...
            {['cash', 'upi', 'bank_transfer', 'razorpay', 'other'].map(m => (
              <button key={m} onClick={() => setMarkMethod(m)}
                className={`text-xs font-bold px-3 py-1.5 rounded-lg border transition-colors capitalize ${
                  markMethod === m ? 'bg-brand-green text-white border-brand-green' : 'bg-white text-gray-900/60 border-brand-green/20 hover:border-brand-green/40'
                }`}>{m.replace('_', ' ')}</button>
            ))}
          </div>
          ...
          <div className="p-6 space-y-4">
            {result.error ? (
              ...
              <>
                    ...
                    {result.payment_link_url && (
                      <>
                        ...
                        <button onClick={() => { navigator.clipboard.writeText(result.payment_link_url); }}
                          className="w-full flex items-center justify-center gap-2 bg-brand-green text-white py-2.5 rounded-xl text-xs font-bold transition-colors">
                ...
                <button onClick={onClose} className="w-full bg-brand-green text-white font-bold py-2.5 rounded-xl hover:bg-brand-green text-white/80 transition-colors">Done</button>
              </>
            )}
          </div>
        ) : (
          <div className="overflow-y-auto flex-1 p-6 space-y-6">
            ...
            <div>
              ...
              <div className="space-y-2">
                {items.map((item, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex gap-3 items-start p-3 bg-[#FDF7E5] rounded-xl border border-brand-green/10">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {allSizes.length > 0 && (
                            <select value={item.variant?.size || ''} onChange={e => updateVariantSize(idx, e.target.value)}
                              className="text-xs border border-brand-green/20 rounded-lg px-2 py-1 bg-white text-gray-900 focus:outline-none">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <input type="number" min={1} value={item.qty} onChange={e => updateQty(idx, e.target.value)}
                              className="w-14 text-xs border border-brand-green/20 rounded-lg px-2 py-1 bg-white text-gray-900 focus:outline-none" />
                          </div>
                ...
                {[['name','Full Name'],['line1','Address Line 1'],['line2','Line 2 (optional)'],['city','City'],['state','State'],['pincode','ZIP']].map(([field, label]) => (
                  <div key={field} className={field === 'line1' ? 'col-span-2' : ''}>
                    ...
                    <input value={address[field] || ''} onChange={e => setAddress(a => ({ ...a, [field]: e.target.value }))}
                      className="w-full text-sm border border-brand-green/15 rounded-xl px-3 py-2 bg-[#FDF7E5] text-gray-900 focus:outline-none focus:border-brand-green/30" />
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleSave} disabled={saving || items.length === 0}
                className="flex-1 px-4 py-2.5 bg-brand-green text-white rounded-xl font-bold hover:bg-brand-green text-white/80 transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
          ...
          <div className="p-6 text-center">
            {refundResult.success ? (
              <>
                ...
                <button onClick={onClose} className="mt-5 w-full bg-brand-green text-white font-bold py-2.5 rounded-xl hover:bg-brand-green text-white/80 transition-colors">Done</button>
              </>
            ) : (
              <>
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  {isFullCancel && shippingDisplay > 0 && (
                    <label className="flex items-center justify-between p-3 bg-[#FDF7E5] rounded-xl border border-brand-green/10 cursor-pointer">
      ...
      <tr style="background:${idx % 2 === 0 ? '#ffffff' : '#FFFAF9'}; ${isCancelled ? 'opacity: 0.6; filter: grayscale(1);' : ''}">
        ...
        <td style="padding:10px 12px;border-bottom:1px solid #F6EFEF;vertical-align:middle;">
          <div style="display:flex;align-items:center;gap:10px;">
            ${absImg ? `<img src="${absImg}" style="width:44px;height:44px;object-fit:cover;border-radius:6px;border:1px solid #f0e0c0;flex-shrink:0;" />` : `<div style="width:44px;height:44px;background:#FDF7E5;border-radius:6px;border:1px solid #f0e0c0;flex-shrink:0;"></div>`}
            <div>
...
<table style="width:100%;border-collapse:collapse;border-bottom:3px solid #08183A;padding-bottom:16px;margin-bottom:20px;">
  <tr>
    <td style="vertical-align:middle;width:50%;">
      <img src="${new URL(logoUrl, window.location.href).href}" style="height:64px;width:auto;object-fit:contain;" alt="VEERABADHRA ENTERPRISE" />
    </td>
...
<div class="print-btn">
  ...
  const sendInvoiceWhatsApp = (order) => {
    ...
    const msg = encodeURIComponent(
      ...
      `*Items:*\n${itemsText}\n\n` +
      `*Total: ₹${order.total}*\n\n` +
      `Thank you for shopping with VEERABADHRA ENTERPRISE!`
    );
    window.open(`https://wa.me/${phone}?text=${msg}`, "_blank");
  };
...
<div class="box">
  <div class="hdr">
    <div class="brand">VEERABADHRA ENTERPRISE</div>
...
<div class="no-print">
  ...
  return (
    <div className="w-full max-w-5xl mx-auto">
          ...
          <button key={s} onClick={() => setStatusFilter(s)}
            className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-bold font-sans capitalize transition-colors ${
              statusFilter === s
                ...
                : "bg-white border border-brand-green/20 text-gray-900/60 hover:border-brand-green/40"
            }`}>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-2xl border border-brand-green/10 p-10 sm:p-12 text-center">
        ...
        <div className="space-y-3 sm:space-y-4">
          {filtered.map((order, i) => (
            <motion.div key={order.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
              className="bg-white rounded-2xl border border-brand-green/10 overflow-hidden">

              <div className="flex items-center gap-2 sm:gap-3 p-3 sm:p-4 lg:p-5 cursor-pointer hover:bg-[#FDF7E5]/30 transition-colors"
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                    ...
                    <span className={`text-[9px] sm:text-[10px] font-bold font-sans px-2 py-0.5 rounded-full ${STATUS_COLORS[order.status] || "bg-[#FDF7E5] text-gray-500"}`}>
                      {order.status}
                    </span>
              ...
              {expanded === order.id && (
                <div className="border-t border-brand-green/5 p-3 sm:p-4 lg:p-5 space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <p className="text-[10px] font-sans text-gray-900/40 uppercase tracking-wider mb-2">Update Status</p>
                      <select value={order.status} onChange={(e) => updateStatus(order.id, e.target.value)}
                        className="w-full px-3 py-2 rounded-xl bg-[#FDF7E5] border border-brand-green/10 text-gray-900 font-sans text-sm focus:outline-none">
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="pt-4 border-t border-brand-green/5">
                        ...
                        <div className="space-y-2">
                          {hist.map((h, i) => (
                            <div key={i} className="bg-[#FDF7E5] rounded-xl px-3 py-2 text-xs text-gray-900/70">
                  ...
                  <div className="pt-4 border-t border-brand-green/5">
                    ...
                    {(() => {
                      ...
                      return (
                        <div className="bg-[#FDF7E5] rounded-xl p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  {/* Action Buttons */}
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 pt-4 border-t border-brand-green/5">
                    ...
                    <button onClick={() => printLabel(order)}
                      className="flex items-center justify-center gap-1.5 bg-brand-green text-white px-3 py-2.5 rounded-xl text-xs font-semibold font-sans hover:bg-brand-green text-white/80 transition-colors">
                    ...
                    <div key={rate.objectId} className="border border-gray-200 rounded-xl p-4 flex items-center justify-between gap-4 hover:border-[#D4AF37] transition-colors">
                      ...
                      <div className="text-right">
                        ...
                        <button onClick={() => purchaseShippoLabel(ratesModal.orderId, rate.objectId)}
                          ...
                          className="mt-1 text-xs bg-brand-green text-white px-3 py-1.5 rounded-lg hover:bg-blue-900 disabled:opacity-50">
