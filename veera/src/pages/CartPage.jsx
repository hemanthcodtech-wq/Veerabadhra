...
export function CartPage() {
  ...
  return (
    <div className="min-h-screen bg-white md:bg-gray-50 font-sans pb-28 md:pb-12">
      {/* ── Brand Hero Header ── */}
      <div className="bg-[#073020] pt-12 pb-10 px-4 md:px-8 md:pt-16 md:pb-14">
      ...
      <div className="bg-white rounded-t-[28px] -mt-5 relative z-10 min-h-screen md:min-h-0 md:bg-transparent md:-mt-8 md:rounded-none md:max-w-6xl md:mx-auto md:px-8">
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center px-8 pt-24 pb-24 gap-5 md:bg-white md:rounded-3xl md:shadow-sm md:mt-4 md:py-32">
            ...
            <button onClick={() => navigate('/')} className="w-full md:w-auto md:px-12 bg-[#073020] text-white font-bold py-4 rounded-2xl shadow-lg shadow-[#073020]/30 hover:scale-105 transition-transform">
              Start Shopping
            </button>
          </div>
        ) : (
                ...
                {items.map((item, idx) => {
                  ...
                  return (
                    <div key={`${item.product.id}-${item.variant?.size || 'default'}`} className="py-4 md:py-6 flex gap-3 md:gap-6 items-center hover:bg-gray-50/50 md:px-4 md:-mx-4 rounded-2xl transition-colors">
                      {/* Product Image */}
                      <div className="w-16 h-16 md:w-24 md:h-24 bg-[#FDF7E5] rounded-2xl overflow-hidden flex-shrink-0 border border-gray-100">
                        <img
                      ...
                      <div className="flex-1 min-w-0 flex flex-col md:flex-row md:items-center md:justify-between gap-2 md:gap-4">
                        <div className="flex-1 pr-2">
                          <Link to={`/product/${item.product.id}`} className="font-bold text-gray-900 text-sm md:text-base leading-tight md:line-clamp-2 hover:text-[#073020] transition-colors">
                            {item.product.name}
                          </Link>
                          <p className="text-xs md:text-sm text-gray-500 mt-0.5 md:mt-1">{item.variant?.size || 'Standard'}</p>
                          <div className="flex items-center gap-2 mt-1.5 md:mt-2">
                            <span className="text-sm md:text-base font-bold text-[#073020]">₹{price.toLocaleString()}</span>
                            {mrp > price && (
                        ...
                        <div className="flex items-center justify-between md:flex-col md:items-end gap-3 md:gap-4">
                          ...
                          <div className="flex items-center border-2 border-gray-200 rounded-full overflow-hidden bg-white">
                            ...
                            <button
                              ...
                              className="w-8 h-8 md:w-9 md:h-9 flex items-center justify-center text-[#073020] hover:bg-red-50 transition-colors"
                            >
              ...
              <div className="mx-4 mt-2 border-t border-gray-100 pt-3 md:hidden">
                {appliedCoupon ? (
                  ...
                  <div className="py-3 space-y-2">
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 border-2 border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#073020]/50"
                      />
                      <button onClick={handleApplyCoupon} className="bg-[#073020] text-white font-bold px-5 py-2.5 rounded-xl text-sm">
                        Apply
                      </button>
                    </div>
                  ...
                  <button onClick={() => setShowCouponInput(true)} className="w-full flex items-center justify-between py-3">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 bg-[#FDF7E5] rounded-xl flex items-center justify-center">
                        <Tag className="w-4 h-4 text-[#073020]" />
                      </div>
            ...
            <div className="md:w-1/3 md:bg-white md:rounded-3xl md:shadow-sm md:p-6 md:border md:border-gray-100 md:sticky md:top-24">
                ...
                {appliedCoupon ? (
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onChange={e => setCouponCode(e.target.value)}
                      placeholder="Enter coupon code"
                      className="flex-1 border-2 border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#073020]/50 transition-colors"
                    />
                    <button onClick={handleApplyCoupon} className="bg-[#073020] hover:bg-[#b51430] text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-colors">
                      Apply
                    </button>
                  </div>
                )}
              ...
              <div className="mx-4 mt-3 md:mx-0 md:mt-0 bg-gray-50 md:bg-transparent rounded-2xl p-4 md:p-0 space-y-3">
                ...
                <div className="border-t border-dashed border-gray-200 pt-3 md:pt-4 flex justify-between font-bold text-gray-900 md:text-lg">
                  <span>To Pay</span>
                  <span className="text-[#073020]">₹{grandTotal.toFixed(0)}</span>
                </div>
              </div>
              ...
              <div className="hidden md:block mt-8">
                <button
                  ...
                  className="w-full bg-[#073020] hover:bg-[#b51430] text-white font-bold py-4 rounded-2xl shadow-lg shadow-[#073020]/30 text-lg active:scale-95 transition-all"
                >
                  Proceed to Checkout
                </button>
                <p className="text-center text-xs text-gray-400 mt-4 flex items-center justify-center gap-1">
                  Secure checkout powered by VEERABADHRA ENTERPRISE
                </p>
              </div>
            </div>
          </div>
      ...
      {items.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-4 py-3 z-50 shadow-2xl md:hidden">
          <div className="flex items-center justify-between gap-4">
            ...
            <button
              onClick={handleCheckout}
              className="flex-1 bg-[#073020] text-white font-bold py-4 rounded-2xl shadow-lg shadow-[#073020]/30 text-base active:scale-95 transition-transform"
            >
              Proceed to Checkout
            </button>
          </div>
          ...
          <div className="bg-white rounded-t-3xl md:rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4 md:zoom-in-95 duration-200">
            ...
            <div className="p-5 md:p-6 space-y-3 pb-8 md:pb-6">
              <button onClick={() => handleDeliveryChoice('shipping')}
                className="w-full flex items-center gap-4 p-4 rounded-2xl border-2 border-gray-100 hover:border-brand-green hover:bg-brand-beige/50 transition-all text-left group">
                <div className="w-12 h-12 bg-brand-beige rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
                  <Truck className="w-6 h-6 text-[#073020]" />
                </div>
                <div>
              ...
              <button onClick={() => handleDeliveryChoice('pickup')}
                className="w-full flex items-center gap-4 p-4 rounded-2xl border-2 border-gray-100 hover:border-brand-green hover:bg-brand-beige/50 transition-all text-left group">
                <div className="w-12 h-12 bg-brand-beige rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
                  <Store className="w-6 h-6 text-[#073020]" />
                </div>
                <div>
      ...
      {showVacationModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[70] p-4">
          <div className="bg-white rounded-3xl w-full max-w-sm shadow-2xl p-6 text-center space-y-4 animate-in zoom-in-95 duration-200">
            ...
            <button onClick={() => setShowVacationModal(false)} className="w-full bg-[#073020] hover:bg-[#b51430] text-white font-bold py-3 rounded-2xl transition-colors">
              Got it
            </button>
          </div>
        </div>
