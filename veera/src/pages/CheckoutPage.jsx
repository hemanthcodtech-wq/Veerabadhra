      ...
      {apiError ? (
        ...
        <div>
          ...
          <input
            ...
            className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none focus:border-brand-green/10 focus:ring-1 focus:ring-brand-gold/30 transition-all"
          />
        </div>
      ) : (
        // ── Autocomplete mode ──────────────────────────────────────────
        <div>
          <MapPin className="w-4 h-4 text-brand-green absolute left-3.5 top-[13px] pointer-events-none" />
          <input
            value={inputVal}
            onChange={handleInput}
            placeholder="Start typing your address..."
            className="w-full bg-gray-50 border border-gray-200 rounded-xl pl-10 pr-4 py-3 text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none focus:border-brand-green/10 focus:ring-1 focus:ring-brand-gold/30 transition-all"
          />
              ...
              {suggestions.map((s) => (
                <li key={s.place_id}>
                  <button
                    ...
                    className="w-full text-left px-4 py-3 text-sm text-gray-700 hover:bg-brand-green hover:text-white flex items-start gap-2.5 border-b border-gray-50 last:border-0 group transition-colors"
                  >
                    <MapPin className="w-4 h-4 text-brand-green group-hover:text-white shrink-0 mt-0.5" />
                    <div>
...
function RazorpayPaymentForm({ isPlacingOrder, handlePlaceOrder, termsAccepted, setTermsAccepted, addressConfirmed, setAddressConfirmed, address, sessionSecondsLeft, onEditAddress, paymentError, onRetry }) {
  ...
  return (
    <div className="space-y-4 max-w-3xl mx-auto">
      ...
      <div className="bg-green-50 border border-green-200 rounded-2xl p-4 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={onEditAddress}
            className="text-[11px] font-bold text-gray-900 underline hover:text-brand-green transition-colors">← Edit Address</button>
        </div>
      ...
      <div className="bg-white/80 p-5 rounded-2xl shadow-sm border border-brand-green/10 space-y-4">
        {paymentError && (
        ...
        <button
          ...
          className={`w-full font-bold text-base rounded-xl py-4 flex items-center justify-center gap-2 transition-all ${
            isPlacingOrder || !termsAccepted || !addressConfirmed
              ...
              : 'bg-brand-green text-white shadow-lg hover:shadow-xl hover:-translate-y-0.5'
          }`}
        >
          {isPlacingOrder ? (
...
export function CheckoutPage() {
  ...
  const renderStepIndicator = () => (
    <div className="flex justify-between items-center mb-6 px-2 bg-white/80 p-3 rounded-xl shadow-sm border border-brand-green/10">
      <div className="flex flex-col items-center cursor-pointer" onClick={() => navigate('/cart')}>
        <div className="w-6 h-6 rounded-full bg-white text-brand-green flex items-center justify-center text-xs font-bold border border-brand-green/10">✓</div>
        <span className="text-[10px] text-gray-900 font-bold mt-1">Cart</span>
      </div>
      <div className={`h-px flex-1 mx-2 ${step >= 2 ? 'bg-brand-green text-white/40' : 'bg-brand-green text-white/20'}`}></div>
      <div className="flex flex-col items-center">
        <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${step >= 2 ? 'bg-white text-brand-green border border-brand-green/10' : 'bg-brand-beige-darker text-gray-900/50 border border-brand-dark-blue/10'}`}>
          {step > 2 ? '✓' : '1'}
        </div>
        <span className={`text-[10px] font-bold mt-1 ${step >= 2 ? 'text-gray-900' : 'text-gray-900/50'}`}>Shipping</span>
      </div>
      <div className={`h-px flex-1 mx-2 ${step >= 2.5 ? 'bg-brand-green text-white/40' : 'bg-brand-green text-white/20'}`}></div>
      <div className="flex flex-col items-center" onClick={() => { if (step === 3 && orderType !== 'pickup') { setStep(2.5); setSessionSecondsLeft(null); clearInterval(sessionTimerRef.current); setAddressConfirmed(false); setTermsAccepted(false); } }} style={{ cursor: step === 3 && orderType !== 'pickup' ? 'pointer' : 'default' }}>
        <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${step >= 2.5 ? 'bg-white text-brand-green border border-brand-green/10' : 'bg-brand-beige-darker text-gray-900/50 border border-brand-dark-blue/10'}`}>
          {step > 2.5 ? '✓' : '2'}
        </div>
        <span className={`text-[10px] font-bold mt-1 ${step >= 2.5 ? 'text-gray-900' : 'text-gray-900/50'}`}>Address</span>
      </div>
      <div className={`h-px flex-1 mx-2 ${step >= 3 ? 'bg-brand-green text-white/40' : 'bg-brand-green text-white/20'}`}></div>
      <div className={`flex flex-col items-center ${step < 3 ? 'opacity-70' : ''}`}>
        <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${step >= 3 ? 'bg-white text-brand-green border border-brand-green/10' : 'bg-brand-beige-darker text-gray-900/50 border border-brand-dark-blue/10'}`}>3</div>
  ...
  return (
    <div className="min-h-screen bg-white font-sans pb-36">
      ...
      <div className="bg-white rounded-t-[28px] -mt-5 relative z-10 min-h-screen p-4 md:p-8 space-y-4 md:max-w-7xl mx-auto">
        ...
        <div className="lg:hidden">
          <button
            ...
            className="w-full flex items-center justify-between bg-white/90 border border-brand-green/10 rounded-2xl px-4 py-3.5 shadow-sm"
          >
            <div className="flex items-center gap-2">
              <ShoppingCart className="w-4 h-4 text-brand-green" />
              <span className="text-sm font-bold text-gray-900">Order Summary</span>
              <span className="text-xs bg-brand-green text-white font-bold px-2 py-0.5 rounded-full">{items.length} item{items.length !== 1 ? 's' : ''}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-sm font-bold text-brand-green">₹{finalTotal.toFixed(2)}</span>
          ...
          {summaryOpen && (
            <div className="mt-2 bg-white/90 border border-brand-green/10 rounded-2xl p-4 shadow-sm space-y-4">
              <div className="space-y-3 max-h-48 overflow-y-auto">
                {items.map(item => (
                  <div key={`${item.product.id}-${item.variant?.size}`} className="flex gap-3">
                    <div className="w-14 h-14 bg-white rounded-xl border border-brand-green/10 p-1 shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm font-bold text-brand-green">₹{((item.variant?.price || item.product.price) * item.qty).toFixed(2)}</p>
                    </div>
                  </div>
                ))}
              </div>
              <div className="border-t border-dashed border-brand-green/10 pt-3 space-y-1.5">
                ...
                {user?.m_coins > 0 && (
                  <div className="flex justify-between items-center text-sm pt-2 mt-1 border-t border-brand-green/5">
                ...
                <div className="flex justify-between font-bold text-gray-900 text-base pt-2 border-t border-brand-green/10">
                  <span>Grand Total</span><span className="text-brand-green">₹{finalTotal.toFixed(2)}</span>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-12 items-start">
          ...
          <div className="space-y-4 max-w-3xl mx-auto">
              ...
              <div className="space-y-3">
                {addresses.map(addr => (
                  <div key={addr.id}>
                    {editingAddr === addr.id ? (
                      /* Inline edit form */
                      <div className="bg-white rounded-2xl border-2 border-brand-green/10 p-4 space-y-3">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                          {[['name','Full Name'],['line2','Line 2 (optional)'],['city','City'],['state','State'],['pincode','ZIP']].map(([key, label]) => (
                            <div key={key} className={key === 'line2' ? 'sm:col-span-2' : ''}>
                              ...
                              <input
                                ...
                                className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-brand-green/10 transition-all"
                              />
                            </div>
                          ))}
                          ...
                          <div ref={editCountryRef} className="relative sm:col-span-2">
                            ...
                            <button type="button" onClick={() => { setEditCountryOpen(o => !o); setEditCountrySearch(''); }}
                              className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm flex items-center gap-2 justify-between focus:outline-none focus:border-brand-green/10 transition-all">
                              ...
                              <div className="absolute z-50 mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden">
                                <div className="p-2 border-b border-gray-100">
                                  <input autoFocus type="text" value={editCountrySearch} onChange={e => setEditCountrySearch(e.target.value)}
                                    placeholder="Search country..." className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand-green/10" />
                                </div>
                                  ...
                                  {COUNTRIES.filter(c => c.name.toLowerCase().includes(editCountrySearch.toLowerCase())).map(c => (
                                    <li key={c.code}>
                                      <button type="button" onClick={() => {
                                        ...
                                        editForm.country === c.name ? 'bg-brand-green text-white/10 font-bold text-gray-900' : 'text-gray-700 hover:bg-gray-50'}`}>
                            ...
                            <div className="flex gap-2">
                              <div ref={editDialRef} className="relative shrink-0">
                                <button type="button" onClick={() => { setEditDialOpen(o => !o); setEditDialSearch(''); }}
                                  className="h-full min-w-[80px] bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm flex items-center gap-1.5 focus:outline-none focus:border-brand-green/10 hover:border-brand-green/10 transition-all">
                                      ...
                                      {COUNTRIES.filter(c => c.name.toLowerCase().includes(editDialSearch.toLowerCase()) || c.dial.includes(editDialSearch)).map(c => (
                                        <li key={c.code}>
                                          <button type="button" onClick={() => { setEditDialCode(c.code); setEditDialOpen(false); }}
                                            className={`w-full text-left px-3 py-2 text-sm flex items-center gap-2.5 transition-colors ${
                                              editDialCode === c.code ? 'bg-brand-green text-white/10 font-bold text-gray-900' : 'text-gray-700 hover:bg-gray-50'}`}>
                                            <span>{flag(c.code)}</span>
                                            <span className="flex-1 truncate">{c.name}</span>
                                            <span className="text-gray-400 font-mono text-xs shrink-0">{c.dial}</span>
                                          </button>
                              ...
                              <input type="text" inputMode="numeric"
                                ...
                                className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-brand-green/10 transition-all"
                              />
                            </div>
                          </div>
                        </div>
                        <button onClick={saveEdit} disabled={savingEdit}
                          className="w-full bg-white text-brand-green font-bold text-sm rounded-xl py-2.5 flex items-center justify-center gap-2 hover:opacity-90 transition-all disabled:opacity-50">
                      ...
                      <button
                        ...
                        className={`w-full text-left p-4 rounded-2xl border-2 transition-all flex items-start gap-3 ${
                          selectedSavedAddress === addr.id ? 'border-brand-green bg-red-50 text-brand-green' : 'border-gray-200 bg-white hover:border-brand-green/10'
                        }`}
                      >
                        <div className={`w-5 h-5 rounded-full border-2 shrink-0 mt-0.5 flex items-center justify-center ${
                          selectedSavedAddress === addr.id ? 'border-brand-green bg-brand-green text-white' : 'border-gray-300'
                        }`}>
                ...
                <button
                  ...
                  className="w-full text-left p-4 rounded-2xl border-2 border-dashed border-gray-200 bg-white hover:border-brand-green/10 transition-all flex items-center gap-3 text-gray-900/60 hover:text-gray-900"
                >
            ...
            {/* New address form */}
            {showNewAddressForm && (
            <div className="bg-white rounded-2xl shadow-sm border border-brand-green/10">
              {addresses.length > 0 && (
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full bg-gray-50 border rounded-xl px-4 py-3 text-sm font-semibold text-gray-900 placeholder:text-gray-400 focus:outline-none transition-all ${
                      fieldErrors.name ? 'border-red-400 focus:border-red-400 focus:ring-1 focus:ring-red-300' : 'border-gray-200 focus:border-brand-green/10 focus:ring-1 focus:ring-brand-gold/30'
                    }`}
                  ...
                  {mapsLoaded ? (
                    ...
                    <input
                      ...
                      className={`w-full bg-gray-50 border rounded-xl px-4 py-3 text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none transition-all ${
                        fieldErrors.line1 ? 'border-red-400 focus:border-red-400 focus:ring-1 focus:ring-red-300' : 'border-gray-200 focus:border-brand-green/10 focus:ring-1 focus:ring-brand-gold/30'
                      }`}
                    />
                  )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input required value={address.city} onChange={e => { setAddress({...address, city: e.target.value}); setFieldErrors(f => ({...f, city: ''})); }}
                      className={`w-full bg-gray-50 border rounded-xl px-4 py-3 text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none transition-all ${
                        fieldErrors.city ? 'border-red-400 focus:border-red-400 focus:ring-1 focus:ring-red-300' : 'border-gray-200 focus:border-brand-green/10 focus:ring-1 focus:ring-brand-gold/30'
                      }`}
                      placeholder="City" />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="text" inputMode="numeric" required value={address.pincode}
                      ...
                      className={`w-full bg-gray-50 border rounded-xl px-4 py-3 text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none transition-all ${
                        fieldErrors.pincode ? 'border-red-400 focus:border-red-400 focus:ring-1 focus:ring-red-300' : 'border-gray-200 focus:border-brand-green/10 focus:ring-1 focus:ring-brand-gold/30'
                      }`}
                    ...
                    <div className="flex gap-2">
                      <div ref={dialRef} className="relative shrink-0">
                        <button type="button" onClick={() => { setDialOpen(o => !o); setDialSearch(''); }}
                          className="h-full min-w-[80px] bg-gray-50 border border-gray-200 rounded-xl px-3 py-3 text-sm flex items-center gap-1.5 focus:outline-none focus:border-brand-green/10 hover:border-brand-green/10 transition-all">
                                ...
                                <li key={c.code}>
                                  <button type="button" onClick={() => { setDialCountryCode(c.code); setDialOpen(false); }}
                                    className={`w-full text-left px-3 py-2.5 text-sm flex items-center gap-2.5 transition-colors ${
                                      dialCountryCode === c.code ? 'bg-brand-green text-white/10 font-bold text-gray-900' : 'text-gray-700 hover:bg-gray-50'}`}>
                      ...
                      <input type="text" inputMode="numeric" required value={address.mobile}
                        ...
                        className={`flex-1 bg-gray-50 border rounded-xl px-4 py-3 text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none transition-all ${
                          fieldErrors.mobile ? 'border-red-400 focus:border-red-400 focus:ring-1 focus:ring-red-300' : 'border-gray-200 focus:border-brand-green/10 focus:ring-1 focus:ring-brand-gold/30'
                        }`}
                ...
                <div ref={countryRef} className="relative">
                  ...
                  <button type="button" onClick={() => { setCountryOpen(o => !o); setCountrySearch(''); }}
                    className={`w-full bg-gray-50 border rounded-xl px-4 py-3 text-sm text-gray-700 focus:outline-none transition-all flex items-center gap-2 justify-between ${
                      fieldErrors.country ? 'border-red-400' : 'border-gray-200 focus:border-brand-green/10 focus:ring-1 focus:ring-brand-gold/30'
                    }`}>
                    <div className="flex items-center gap-2 min-w-0">
                        ...
                        {displayCountries.filter(c => c.name.toLowerCase().includes(countrySearch.toLowerCase())).map(c => (
                          <li key={c.code}>
                            <button type="button" onClick={() => { setAddress({...address, country: c.name}); setDialCountryCode(c.code); setCountryOpen(false); setFieldErrors(f => ({...f, country: ''})); }}
                              className={`w-full text-left px-3 py-2.5 text-sm flex items-center gap-2.5 transition-colors ${
                                address.country === c.name ? 'bg-brand-green text-white/10 text-gray-900 font-bold' : 'text-gray-700 hover:bg-gray-50'}`}>
              ...
              {scheduleSlots.length > 0 && orderType !== 'pickup' && (
                <div className="px-4 pt-2 pb-2 sm:px-6">
                  <div className="bg-[#FDF7E5] border border-brand-green/20 rounded-2xl p-4 space-y-3">
        ...
        {step === 3 && orderType === 'pickup' && (
          <div className="space-y-4 max-w-3xl mx-auto">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-brand-green/10 p-5 space-y-4">
                ...
                <div className="flex gap-2">
                  ...
                  <div ref={pickupDialRef} className="relative shrink-0">
                    <button
                      ...
                      className="h-full min-w-[90px] bg-gray-50 border border-gray-200 rounded-xl px-3 py-3 text-sm flex items-center gap-1.5 focus:outline-none focus:border-brand-green/10 hover:border-brand-green/10 transition-all"
                    >
                      ...
                      <div className="absolute z-50 mt-1 left-0 w-64 bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden">
                        <div className="p-2 border-b border-gray-100">
                          <input
                            ...
                            className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand-green/10"
                          />
                        </div>
                        <ul className="max-h-52 overflow-y-auto">
                          {COUNTRIES.filter(c =>
                            ...
                            <li key={c.code}>
                              <button
                                ...
                                className={`w-full text-left px-3 py-2.5 text-sm flex items-center gap-2.5 transition-colors ${
                                  pickupDialCode === c.code ? 'bg-brand-green text-white/10 font-bold text-gray-900' : 'text-gray-700 hover:bg-gray-50'
                                }`}
                              >
                  ...
                  <input
                    ...
                    placeholder="Phone number"
                    className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none focus:border-brand-green/10 focus:ring-1 focus:ring-brand-gold/30 transition-all"
                  />
                </div>
            ...
            <button
              ...
              className={`w-full font-bold text-base rounded-xl py-4 flex items-center justify-center gap-2 transition-all ${
                isPlacingOrder || !pickupTermsAccepted
                  ? 'opacity-50 cursor-not-allowed bg-gray-200 text-gray-400'
                  : 'bg-white text-brand-green shadow-lg shadow-brand-dark-blue/20 hover:shadow-xl hover:-translate-y-0.5'
              }`}
            >
              {isPlacingOrder
        ...
        {step === 3 && orderType !== 'pickup' && (
          ...
          <div className="hidden lg:block lg:col-span-4 sticky top-24">
            <div className="bg-white/80 p-6 rounded-3xl shadow-sm border border-brand-green/10">
              ...
              <div className="space-y-4 max-h-[40vh] overflow-y-auto hide-scrollbar pr-2 mb-6">
                {items.map(item => (
                  <div key={`${item.product.id}-${item.variant?.size}`} className="flex gap-4">
                    <div className="w-16 h-16 bg-white rounded-xl border border-brand-green/10 p-1 shrink-0">
                      <img src={item.product.images?.[0] || item.product.image_url} alt="" className="w-full h-full object-contain" />
                    </div>
                    <div>
                      ...
                      <p className="text-sm font-bold text-brand-green mt-1">₹{((item.variant?.price || item.product.price) * item.qty).toFixed(2)}</p>
                    </div>
                  </div>
                ))}
              </div>

              <div className="border-t border-dashed border-brand-green/10 pt-4 mb-6">
                ...
                {user?.m_coins > 0 && (
                  <div className="flex justify-between items-center text-sm mb-3 pb-2 border-b border-brand-green/5">
                ...
                <div className="flex justify-between font-bold text-gray-900 text-xl pt-2 border-t border-brand-green/10">
              ...
              {(step === 2 || step === 2.5) ? (
                <button 
                  ...
                  className={`w-full bg-brand-green text-white font-bold text-base rounded-xl py-4 shadow-lg shadow-brand-green/30 hover:shadow-xl hover:bg-brand-green/90 hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2 ${step === 2 && pickupEnabled ? 'opacity-40 cursor-not-allowed' : ''}`}
                >
                  Proceed to Payment
                </button>
              ) : null}
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-brand-beige/95 backdrop-blur-md border-t border-brand-green/10 p-4 pb-safe z-50 shadow-[0_-8px_30px_rgba(0,0,0,0.04)] mx-auto w-full">
            ...
            <div>
              <>
                ...
                <div className="flex flex-col">
                  {appliedCoupon && <span className="text-[10px] text-brand-green font-bold -mb-1">Code applied: {appliedCoupon.code}</span>}
      ...
      {isPlacingOrder && (
        <div className="fixed inset-0 z-[100] bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center">
          <div className="w-14 h-14 border-4 border-brand-green/10 border-t-brand-gold rounded-full animate-spin mb-4" />
      ...
      {orderSuccess && (
        <div ref={overlayRef} className="fixed inset-0 z-[100] bg-white/90 backdrop-blur-sm flex flex-col items-center justify-center w-full h-full">
          <div className="flex flex-col items-center gap-4 px-6 text-center">
            <div ref={iconRef} className="w-24 h-24 bg-brand-green text-white rounded-full flex items-center justify-center shadow-lg">
              <CheckCircle className="w-12 h-12 text-white" strokeWidth={2.5} />
            </div>
            <h2 ref={textRef} className="text-2xl font-serif font-bold text-gray-900">Order Confirmed!</h2>
            <p className="text-sm text-gray-600 max-w-sm leading-relaxed">
              Thank you for placing your order with VEERABADHRA ENTERPRISE. We're delighted to begin preparing your selection and will keep you updated throughout its journey to you.
            </p>
            {transactionId && (
