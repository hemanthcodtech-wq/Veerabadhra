          ...
          {suggestions.map(s => (
            <li key={s.place_id}>
              <button type="button" onMouseDown={(e) => { e.preventDefault(); handleSelect(s); }}
                className="w-full text-left px-4 py-3 text-sm text-gray-700 hover:bg-brand-green text-white/5 flex items-start gap-2.5 border-b border-gray-50 last:border-0">
      ...
      <div className="bg-white rounded-t-3xl md:rounded-2xl w-full max-w-lg p-6 max-h-[92vh] overflow-y-auto">
        ...
        <label className="flex items-center gap-2.5 mt-4 cursor-pointer">
          <div onClick={() => setForm(f => ({ ...f, is_default: !f.is_default }))}
            className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-colors ${form.is_default ? 'bg-brand-green text-white border-brand-green/10' : 'border-gray-300'}`}>
        ...
        <button onClick={handleSubmit} disabled={saving}
          className="w-full mt-5 bg-brand-green text-white font-bold py-3.5 rounded-xl text-sm hover:bg-gray-600 transition-colors disabled:opacity-60">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={() => { setEditingAddress(null); setShowForm(true); }}
            className="flex items-center gap-2 text-sm font-bold text-white bg-brand-green text-white px-4 py-2.5 rounded-xl hover:bg-gray-600 transition-colors shadow-sm">
          ...
          {addresses.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 gap-4">
              ...
              <button onClick={() => setShowForm(true)}
                className="bg-brand-green text-white text-sm font-bold px-6 py-2.5 rounded-xl hover:bg-gray-600 transition-colors">
                Add Address
              </button>
            </div>
          ) : (
