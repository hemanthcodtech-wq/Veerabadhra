    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end md:items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
        ...
        <button onClick={() => onSave(form)}
          className="w-full mt-4 bg-brand-green text-white font-bold py-3 rounded-xl text-sm hover:bg-gray-600 transition-colors">
          Save Address
        </button>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="max-w-4xl mx-auto">
        {/* User Profile Header */}
        <div className="bg-brand-green text-white px-6 pt-8 pb-8 md:rounded-3xl md:mt-6 rounded-b-[2.5rem] shadow-sm">
          ...
          {tabs.map((tab) => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={`flex-1 py-2.5 text-xs md:text-sm font-bold rounded-lg transition-colors ${
                activeTab === tab.id ? 'bg-brand-green text-white shadow-sm' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'
              }`}>
              {tab.label}
            </button>
          ))}
        ...
        {activeTab === 'overview' && (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                { label: 'Orders', value: orders.length, color: 'bg-gray-50 text-brand-green' },
          ...
          <div className="space-y-3">
            {orders.length === 0 ? (
              <div className="text-center py-16">
                ...
                <Link to="/" className="mt-3 inline-block text-xs text-brand-green font-bold">Start Shopping →</Link>
              </div>
            ) : (
              orders.map((order) => (
        ...
        {activeTab === 'addresses' && (
          <div className="space-y-3">
            <button onClick={() => setShowAddressModal(true)}
              className="w-full flex items-center justify-center gap-2 border-2 border-dashed border-orange-300 rounded-xl py-4 text-brand-green text-sm font-semibold hover:bg-gray-50 transition-colors">
