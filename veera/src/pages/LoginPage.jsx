...
function ForgotPassword({ onBack, dark = false }) {
  ...
  const inputCls = dark
    ...
    : 'w-full bg-white border border-[#D4AF37]/30 rounded-xl px-4 pl-11 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:border-[#D4AF37] focus:ring-4 focus:ring-[#D4AF37]/20 transition-all duration-300 shadow-sm';
  ...
  const btnCls = dark
    ...
    : 'w-full bg-gradient-to-r from-[#D4AF37] to-[#C5A030] text-[#073020] font-bold py-3.5 rounded-xl text-sm hover:shadow-[0_0_20px_rgba(212,175,55,0.4)] hover:-translate-y-0.5 transition-all duration-300 disabled:opacity-60 disabled:hover:translate-y-0 shadow-md';
  const errCls = dark
  ...
  const handleOtpChange = (val, idx, rawVal) => {
    val = val.replace(/\D/g, '');
    if (!val && rawVal !== '') return;
    if (val.length > 1) {
      const next = [...otp];
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        <h4 className={`font-bold tracking-widest uppercase text-xs mb-1 ${dark ? 'text-brand-orange' : 'text-brand-green'}`}>Reset Password</h4>
          ...
          <div className="flex justify-center gap-2">
            {otp.map((digit, idx) => (
              <input key={idx} ref={el => otpRefs.current[idx] = el}
                ...
                onChange={e => handleOtpChange(e.target.value, idx, e.target.value)}
                onKeyDown={e => handleOtpKeyDown(e, idx)}
                className={`w-10 h-12 text-center text-xl font-bold rounded-xl border-2 focus:outline-none transition-colors ${
                  dark ? 'bg-transparent border-white/20 text-white focus:border-[#D4AF37]' : 'bg-white border-brand-green/10 text-gray-900 focus:border-brand-green/10'
                }`}
              />
            ))}
          </div>
...
export function LoginPage() {
  ...
  return (
    <>
      {/* DESKTOP VIEW */}
      <div className="hidden lg:flex h-screen w-full font-sans bg-[#073020] relative items-center justify-center overflow-hidden py-2">
        ...
        <div className="w-full max-w-md bg-[#FDF7E5]/95 backdrop-blur-2xl rounded-[2rem] shadow-[0_0_50px_rgba(212,175,55,0.2)] border border-[#D4AF37]/30 flex flex-col items-center justify-center px-8 py-10 relative z-10">
          <motion.div
          ...
          >
            <div className="flex justify-center mb-6">
              <Link to="/">
                <img src={logoImg} alt="VEERABADHRA ENTERPRISE" className="h-[140px] w-auto drop-shadow-[0_0_20px_rgba(212,175,55,0.5)] hover:scale-105 transition-transform" />
              </Link>
            </div>
            
            {!showForgot && (
              <div className="text-center mb-6">
                <h2 className="text-3xl font-serif font-extrabold tracking-tight text-[#073020] mb-1">Welcome Back</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-[#D4AF37]/30 rounded-xl px-4 pl-11 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:border-[#D4AF37] focus:ring-4 focus:ring-[#D4AF37]/20 transition-all duration-300 shadow-sm"
                  />
                </div>
              </div>
              ...
              {displayError && (
                ...
                <motion.button
                  ...
                  className="w-full bg-gradient-to-r from-[#D4AF37] to-[#C5A030] text-[#073020] font-bold py-3.5 rounded-2xl text-sm transition-transform active:scale-95 disabled:opacity-60 mt-4 shadow-[0_0_15px_rgba(212,175,55,0.3)]"
                >
              ...
              <button type="button" onClick={() => loginWithGoogle()} className="w-full bg-white/50 backdrop-blur-md border border-[#D4AF37]/50 text-[#073020] font-bold py-3.5 rounded-xl flex items-center justify-center gap-3 text-sm hover:bg-white transition-all duration-300 shadow-sm hover:shadow-md">
              ...
              <div className="mt-8 text-center text-sm text-gray-600 font-medium">
                New to Veerabadhra?{' '}
                <Link to={`/signup?redirect=${redirect}`} className="text-[#073020] font-bold hover:text-[#D4AF37] transition-colors underline decoration-2 underline-offset-4">
                  Create an account
                </Link>
              </div>
              </>
      ...
      {/* MOBILE VIEW */}
      <div className="lg:hidden flex flex-col h-screen w-full font-sans bg-[#073020] relative">
        <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1605806616949-1e87b487cb2a?q=80&w=2070')] bg-cover bg-center opacity-30 mix-blend-overlay"></div>
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-[#073020]/80 to-[#073020]"></div>
        ...
        <div className="flex-1 flex items-center justify-center relative z-10 px-6">
          <motion.div
            ...
            className="w-full max-w-sm bg-[#FDF7E5]/95 backdrop-blur-xl rounded-[32px] p-8 shadow-[0_0_40px_rgba(212,175,55,0.15)] border border-[#D4AF37]/20"
          >
            <div className="flex justify-center mb-6">
              <Link to="/">
                <img src={brandLogo} alt="VEERABADHRA ENTERPRISE" className="h-[110px] w-auto drop-shadow-[0_0_15px_rgba(212,175,55,0.5)] hover:scale-105 transition-transform" />
              </Link>
            </div>
          
          {!showForgot && (
              <div className="text-center mb-6">
                <h2 className="text-2xl font-serif font-bold tracking-tight text-[#073020] mb-2">Welcome Back</h2>
                <p className="text-xs text-gray-600 font-medium">Continue your spiritual journey</p>
              </div>
            )}
          ...
          <div>
            ...
            <div className="relative group">
              ...
              <input
                name="email" type="email" value={form.email} onChange={handleChange} required
                placeholder="you@example.com"
                className="w-full bg-white border border-gray-200 shadow-inner rounded-xl px-4 pl-11 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:border-[#073020] focus:ring-4 focus:ring-[#073020]/10 focus:bg-white hover:border-gray-300 transition-all duration-300 shadow-sm"
              />
            </div>
          </div>
        ...
        <div className="flex items-center gap-3 w-full max-w-sm my-3 z-10">
          <div className="h-px bg-[#D4AF37]/30 flex-1"></div>
          <span className="text-[#D4AF37] text-[10px] tracking-wider uppercase font-semibold">OR</span>
          <div className="h-px bg-[#D4AF37]/30 flex-1"></div>
        </div>

        <button type="button" onClick={() => loginWithGoogle()} className="relative z-10 w-full max-w-sm bg-white/50 backdrop-blur-md border border-[#D4AF37]/50 text-[#073020] font-bold py-2.5 rounded-xl flex items-center justify-center gap-3 text-sm hover:bg-white transition-colors shadow-sm">
          <svg viewBox="0 0 24 24" width="18" height="18" xmlns="http://www.w3.org/2000/svg"><g transform="matrix(1, 0, 0, 1, 27.009001, -39.238998)"><path fill="#4285F4" d="M -3.264 51.509 C -3.264 50.719 -3.334 49.969 -3.454 49.239 L -14.754 49.239 L -14.754 53.749 L -8.284 53.749 C -8.574 55.229 -9.424 56.479 -10.684 57.329 L -10.684 60.329 L -6.824 60.329 C -4.564 58.239 -3.264 55.159 -3.264 51.509 Z"/><path fill="#34A853" d="M -14.754 63.239 C -11.514 63.239 -8.804 62.159 -6.824 60.329 L -10.684 57.329 C -11.764 58.049 -13.134 58.489 -14.754 58.489 C -17.884 58.489 -20.534 56.379 -21.484 53.529 L -25.464 53.529 L -25.464 56.619 C -23.494 60.539 -19.444 63.239 -14.754 63.239 Z"/><path fill="#FBBC05" d="M -21.484 53.529 C -21.734 52.809 -21.864 52.039 -21.864 51.239 C -21.864 50.439 -21.724 49.669 -21.484 48.949 L -21.484 45.859 L -25.464 45.859 C -26.284 47.479 -26.754 49.299 -26.754 51.239 C -26.754 53.179 -26.284 54.999 -25.464 56.619 L -21.484 53.529 Z"/><path fill="#EA4335" d="M -14.754 43.989 C -12.984 43.989 -11.404 44.599 -10.154 45.789 L -6.734 42.369 C -8.804 40.429 -11.514 39.239 -14.754 39.239 C -19.444 39.239 -23.494 41.939 -25.464 45.859 L -21.484 48.949 C -20.534 46.099 -17.884 43.989 -14.754 43.989 Z"/></g></svg>
          Continue with Google
        </button>
        ...
        <p className="text-center text-xs text-gray-500 mt-4 relative z-10">
          ...
          <Link to="/signup" className="text-[#D4AF37] font-bold hover:text-[#073020] transition-colors underline decoration-2 underline-offset-4">
            Create Account
          </Link>
        </p>
