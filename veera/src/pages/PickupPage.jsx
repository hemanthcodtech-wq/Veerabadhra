...
function StripeCardForm({ onReady }) {
  ...
  return (
    <div className="p-3 border border-brand-green/10 rounded-xl bg-white">
      <CardElement options={{
        style: {
  ...
  const renderStepIndicator = () => (
    <div className="flex justify-between items-center mb-6 px-2 bg-white/80 p-3 rounded-xl shadow-sm border border-brand-green/10">
      ...
      <div className="flex flex-col items-center cursor-pointer" onClick={() => step > 1 && setStep(1)}>
        <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold border ${step >= 1 ? 'bg-white text-brand-green border-brand-green/10' : 'bg-brand-beige-darker text-gray-900/60 border-brand-dark-blue/10'}`}>
          {step > 1 ? '✓' : '1'}
        </div>
        <span className={`text-[10px] font-bold mt-1 ${step >= 1 ? 'text-gray-900' : 'text-gray-900/60'}`}>Login</span>
      </div>
      <div className={`h-px flex-1 mx-2 ${step > 1 ? 'bg-white' : 'bg-brand-green text-white/30'}`}></div>

      <div className="flex flex-col items-center cursor-pointer" onClick={() => step > 2 && setStep(2)}>
        <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold border ${step >= 2 ? 'bg-white text-brand-green border-brand-green/10' : 'bg-brand-beige-darker text-gray-900/60 border-brand-dark-blue/10'}`}>
          {step > 2 ? '✓' : '2'}
        </div>
        <span className={`text-[10px] font-bold mt-1 ${step >= 2 ? 'text-gray-900' : 'text-gray-900/60'}`}>Details</span>
      </div>
      <div className={`h-px flex-1 mx-2 ${step > 2 ? 'bg-white' : 'bg-brand-green text-white/30'}`}></div>

      <div className="flex flex-col items-center">
        <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold border ${step >= 3 ? 'bg-white text-brand-green border-brand-green/10' : 'bg-brand-beige-darker text-gray-900/60 border-brand-dark-blue/10'}`}>
          3
        </div>
    ...
    <div className="min-h-screen bg-brand-beige pb-36">
      ...
      <div className="p-4 md:p-8 md:max-w-7xl mx-auto mt-6">
        <button 
          ...
          className="flex items-center text-sm font-bold text-gray-900 hover:text-brand-green transition-colors mb-6"
        >
          ...
          <div className="lg:col-span-8">
            ...
            <div className="lg:hidden mb-4">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs bg-brand-green text-white/10 text-brand-green font-bold px-2 py-0.5 rounded-full">{items.length} item{items.length !== 1 ? 's' : ''}</span>
                </div>
                <div className="flex items-center gap-2">
            ...
            {step === 1 && (
              <div className="space-y-4 max-w-2xl mx-auto">
                <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2 mb-6">
                  <div className="w-8 h-8 rounded-full bg-brand-green text-white/10 flex items-center justify-center">
                    <UserCircle2 className="w-4 h-4 text-brand-green" />
                  </div>
                  Account Details
                </h2>
                <div className="bg-white/80 p-6 rounded-3xl shadow-sm border border-brand-green/10 flex flex-col items-center justify-center text-center">
                  <UserCircle2 className="w-16 h-16 text-brand-green mb-4" />
                  ...
                  <div className="w-full max-w-sm space-y-3 flex flex-col items-center">
                    <button 
                      ...
                      className="w-full bg-white text-brand-green font-bold py-3 rounded-xl shadow-lg shadow-brand-dark-blue/20 hover:shadow-xl hover:-translate-y-0.5 transition-all"
                    >
                      Login to your account
                    </button>
                    <div className="flex items-center w-full gap-3 py-2">
                      <div className="h-px bg-brand-green text-white/20 flex-1"></div>
            ...
            {step === 2 && (
              <div className="space-y-4 max-w-3xl mx-auto">
                ...
                <div className="bg-white/80 p-6 rounded-2xl shadow-sm border border-brand-green/10 relative overflow-hidden">
                  ...
                  <div className="space-y-5">
                    <div>
                      ...
                      <input required value={details.name} onChange={e => setDetails({...details, name: e.target.value})} className="w-full text-lg font-bold text-gray-900 border-b-2 border-gray-100 py-1 focus:outline-none focus:border-brand-green/10 transition-colors bg-transparent" placeholder="Full Name for Pickup" />
                    </div>
                    <div>
                      <label className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1 block">Mobile</label>
                      <input type="text" maxLength={10} required value={details.mobile} onChange={e => setDetails({...details, mobile: e.target.value.replace(/\D/g, '')})} className="w-full text-base text-gray-700 border-b border-gray-200 py-1 focus:outline-none focus:border-brand-green/10 transition-colors bg-transparent" placeholder="Mobile Number" />
                    </div>
                    <div className="mt-4 p-4 bg-white border border-brand-dark-blue/10 rounded-xl">
                      <p className="text-sm text-gray-900 font-bold flex items-center gap-2"><Store className="w-4 h-4 text-brand-green"/> Pickup Location</p>
                      <p className="text-xs text-gray-900/80 mt-1">VEERABADHRA ENTERPRISE Store, Main Market, City Center</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {step === 3 && (
              <div className="space-y-4 max-w-3xl mx-auto">
                ...
                <div className="bg-white/80 p-5 rounded-2xl shadow-sm border border-brand-green/10">
            ...
            <div className="bg-white/80 p-6 rounded-3xl shadow-sm border border-brand-green/10">
                ...
                {items.map(item => (
                  <div key={`${item.product.id}-${item.variant?.size}`} className="flex gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-brand-green mt-1">${(item.variant?.price || item.product.price) * item.qty}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {step === 1 ? (
                ...
                <button 
                  ...
                  className="w-full bg-white text-brand-green font-bold text-base rounded-xl py-4 shadow-lg shadow-brand-dark-blue/20 hover:shadow-xl hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2"
                >
                  Proceed to Payment
                </button>
              ) : (
                <button
                  ...
                  className={`w-full font-bold text-base rounded-xl py-4 flex items-center justify-center gap-2 transition-all ${
                    isPlacingOrder ? 'opacity-70 cursor-not-allowed bg-brand-beige-darker text-gray-900/50' : 'bg-white text-brand-green shadow-lg shadow-brand-dark-blue/20 hover:shadow-xl hover:-translate-y-0.5'
                  }`}
                >
                  {isPlacingOrder ? (
                    <div className="flex items-center gap-2">
          ...
          {step === 1 ? (
            ...
            <button
              ...
              className={`w-full font-bold text-base rounded-xl py-4 shadow-lg flex items-center justify-center gap-2 transition-all ${
                isPlacingOrder ? 'opacity-70 cursor-not-allowed bg-brand-beige-darker text-gray-900/50 shadow-none' : 'bg-white text-brand-green shadow-brand-dark-blue/20 hover:shadow-xl hover:-translate-y-0.5'
              }`}
            >
              {isPlacingOrder ? (
                <div className="flex items-center gap-2">
